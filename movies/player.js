...
return `Auto (${video.videoWidth}×${video.videoHeight})`;
}
return "Auto";
}
...
window.hlsInstance = new Hls({
  maxBufferLength: 120,        // seconds to keep buffered
  maxMaxBufferLength: 130,     // hard cap
  backBufferLength: 20,       // keep past data (seek back)
  enableWorker: true,
...
// ----- HLS AUTO LABEL -----
autoItem.textContent = getVideoResolutionText();
...
video.addEventListener("playing", () => {
  if (startupQualityShown) return;

  const w = video.videoWidth;
  const h = video.videoHeight;

  if (w > 0 && h > 0) {
    showToast(`Quality: ${w}×${h}`);
    startupQualityShown = true;
  }
});
...
const defaultUrl = slug
  ? `https://video.manojwrites.xyz/${slug}/en.vtt`
  : "";

const url = prompt(
  "Enter subtitle URL (.vtt or .srt):",
  defaultUrl
);
//------
...
if (lvl.width > 0 && lvl.height > 0) {
    showToast(`Quality: ${lvl.width}×${lvl.height}`);
    updateAutoQualityLabel();
    window._lastHlsLevel = current;
}
}
