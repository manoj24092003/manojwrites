...
if ($type === 'recent') {
    ...
    $sql = "SELECT id, name, slug, genre, imdb, type, year
            FROM movie
            ORDER BY id DESC";
...
elseif ($genre) {
    ...
    // split label like "Anime/Animation"
    $parts = explode('/', strtolower($genre));
...
<title>Manoj Writes - Contents</title>  
...
<h2 style="color:#fff; font-weight:400; margin-bottom:1rem;">
    <?php if ($genre) {echo htmlspecialchars(ucwords($genre));} else { echo "Movies"; }?>
</h2>

    <?php
$watchList = $mainResult;
?>

<?php if ($watchList->num_rows === 0): ?>
    <p style="color:#aaa;font-size:1rem;">No movies found.</p>
<?php endif; ?>
...
{
    $tstmt = $con->prepare("
    SELECT genre, tags FROM movie ORDER BY RAND() LIMIT 30
");
$tstmt->execute();
...
<h4 style='font-weight:400;padding-bottom:5px;'>You May Also Like</h4>
