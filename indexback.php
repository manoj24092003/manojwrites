...
<header class="header">
    ...
    <div class="logo"><i class="bi bi-tv-fill tv-logo"></i></div>  
...
<section class="hero" style="padding: 1rem;">  
    <?php  
    // pick 5 random top movies each refresh  
    $topMovies = [];  
    $sql = "  
        SELECT id, name, poster, posterpng, imdb, year, duration, genre, description, slug    
        FROM movie  
        WHERE is_top = 1  
        ORDER BY RAND()  
        LIMIT 5  
        ...
        <?php foreach ($topMovies as $i => $m): ?>  
            ...
            >  
                ...
                <div class="hero-banner-content">  
                    <?php if (!empty($m['posterpng'])): ?>  
                        <img  
                            src="<?= htmlspecialchars($m['posterpng']) ?>"  
                            alt="<?= htmlspecialchars($m['name']) ?>"  
                            class="hero-logo"  
                        >  
                    <?php else: ?>  
    ...
    <h4 style='font-weight:400;'>Recently Added</h4>  
        ...
        $duration = ((int)$raw) * 60;
            ...
            <a href="movies/<?= htmlspecialchars($cw['slug']); ?>" style="text-decoration:none;">
                <div class="movie-card sk" style="position:relative;">
                    ...
                    <img src="<?= htmlspecialchars($cw['poster']); ?>" alt="Poster">
...
?>
    ...
    "Slice of Life" => [
        "label" => "Slice of Life",
        "match" => ["Slice of Life","Journey","feel-good"]
    ],
    ...
    foreach ($genres as $key =>$g) {  
        ...
        // Print heading  
        echo "<h4 style='margin-top:20px;font-weight:400;'>{$g['label']}</h4>";  
    ...
    $sql = "SELECT id, poster, genre, imdb, name, slug 
            FROM movie 
...
<section class="about">  
        <div class="aboutbox">  
            ...
            <div class="abouttext hidden hiddenbottom">  
                ...
                <p>  
                    Some stories whisper before they speak. Some worlds wait quietly, unseen. <span style="font-style: italic;">MANOJ</span> wanders in those shadows, gathering fragments of thought, memory, and imagination, and weaving them into tales that linger in the corners of the mind. Every sentence is a doorway, every story a secret—inviting readers to step inside, if they dare.  
                </p>  
            </div>  
        </div>
    </section>  
